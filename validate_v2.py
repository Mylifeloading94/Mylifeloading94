...
V2 = {'exits.stop_scale':2.5,'exits.tp_model':'full','exits.tp1_r':1.25,'exits.tp2_r':1.25,
      ...
      'score.threshold':0.0,'risk.min_rr':1.2,'risk.max_daily_trades':6,
...
b = pd.DataFrame(blocks); b.to_csv('xauusd_bot/reports/v2_walkforward.csv', index=False)
traded = b[b['n'] >= 15]
...
T.to_csv('xauusd_bot/reports/v2_all_trades.csv', index=False)
r = T['r_multiple']
gl = -r[r<0].sum()
...
json.dump({'mc':mc,'wf_stability':wf}, open('xauusd_bot/reports/validation_v2.json','w'),
          indent=2, default=str)
from xauusd_bot.deployment_gate import run_checks, render
print("\n" + render(run_checks('xauusd_bot/reports/validation_v2.json', run_tests=False)))
